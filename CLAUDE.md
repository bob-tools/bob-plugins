...
## ✍️ Написание бренда — канон: **BoB**

Бренд пишется **`BoB`** (заглавная B, строчная o, заглавная B): `BoB`, `BoB Chat`,
`BoB Tools`, `BoB Assistant`, `BoB Pro`/`BoB Max`. Формы «BOB» капсом и «Bob» —
ошибка, чинить в любом пользовательском тексте, который попался под руку.

Не трогаем: юридическое имя «BOB TOOLS LLP», идентификаторы (`bob.tools.chat`,
`tools.bob.one`, `bob_records`, `BOB_LINE_URL`), домены/URL, пути и имена файлов.
Полное правило — `../CLAUDE.md`, раздел «Написание бренда».
